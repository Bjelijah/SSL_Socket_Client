...
typedef struct {
	...
	JNIEnv * env;
	jobject callback_obj;
	jmethodID on_connect_method;
	int transDataLen;
}TRANS_T;

TRANS_T* g_transMgr = NULL;
...
void getCamCallback(const char *str,int len){

}

void getRecordFileCallback(const char *str,int len){
...
int on_my_ack_res(int msgCommand,void * res,int len){
	...
	case 0x102:
		LOGI("%s",str);
		getCamCallback(str,len);
		break;
	case 0x104:
		LOGI("%s",str);
		getRecordFileCallback(str,len);
		break;
	default:
		break;
...
JNIEXPORT void JNICALL Java_com_howell_jni_JniUtil_getCam
...
JNIEXPORT void JNICALL Java_com_howell_jni_JniUtil_getRecordFiles
  (JNIEnv *env, jclass, jstring jsonStr, jint len){
	const char *_jsonStr = env->GetStringUTFChars(jsonStr,0);
